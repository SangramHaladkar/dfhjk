...
public class ContactFragment extends AppBaseFragment {
    ...
    public void getContactInformation() {
        try {
            ...
            if (c.getCount() > 0) {
                while (c.moveToNext()) {
                    ed_Email.setText(c.getString(16));

                    String strMobNum[] = c.getString(18).split("@");

                    if (c.getString(17).toLowerCase().equals("true"))
                        switchNotifyByEMail.setChecked(true);

                    ed_MobilePhone.setText(strMobNum[0]);

                    if (c.getString(19).toLowerCase().equals("true"))
                        switchNotifyBySMS.setChecked(true);

                    String strSMSAddress = "";
                    if (c.getString(18).contains("@txt.att.net"))
                        strSMSAddress = "AT&T @txt.att.net";
                    if (c.getString(18).contains("@tmomail.net"))
                        strSMSAddress = "T-Mobile @tmomail.net";
                    if (c.getString(18).contains("@vtext.com"))
                        strSMSAddress = "Verizon @vtext.com";
                    if (c.getString(18).contains("@message.alltel.com"))
                        strSMSAddress = "Alltel @message.alltel.com";
                    if (c.getString(18).contains("@myboostmobile.com"))
                        strSMSAddress = "Boost Mobile @myboostmobile.com";
                    if (c.getString(18).contains("@messaging.nextel.com"))
                        strSMSAddress = "Nextel @messaging.nextel.com";
                    if (c.getString(18).contains("@messaging.sprintpcs.com"))
                        strSMSAddress = "Sprint PCS @messaging.sprintpcs.com";
                    if (c.getString(18).contains("@email.uscc.net"))
                        strSMSAddress = "US Cellular @email.uscc.net";
                    if (c.getString(18).contains("@vmobl.com"))
                        strSMSAddress = "Virgin Mobile USA @vmobl.com";
                    if (c.getString(18).contains("@mymetropcs.com"))
